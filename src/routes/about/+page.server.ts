...
// פאנל ניהול אתרי הרשת (לשעבר /admin) יושב בכרטיסייה "ניהול הרשת" שבדף האודות:
// סופר-אדמין מקבל שם את הטבלה הניתנת לעריכה ואת הפעולות שלמטה; כל השאר —
// תצוגה לקריאה בלבד (NetworkAdmins).
...
async function requireSuperAdmin(locals: App.Locals) {
	...
	if (!session?.user) throw redirect(302, '/login?redirect=' + encodeURIComponent('/about?tab=network'));
...
export const load: PageServerLoad = async ({ locals }) => {
	// דף האודות פתוח לכולם — נתוני הפאנל נטענים רק לסופר-אדמין
	const session = await locals.auth();
	const jwt = (session?.user as { strapiJwt?: string } | undefined)?.strapiJwt ?? '';
	if (!session?.user || !isSuperAdmin(session.user.email) || !jwt) return { panel: null };

	// הנתונים מוחזרים כהבטחה ולא ב-await: SvelteKit מזרים אותם לדף. כך הלחיצה
	// על "צוות הנהלה" פותחת את הדף מיד — השלד על המסך — והטבלה מתמלאת
