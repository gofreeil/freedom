...
<div class="min-h-[80vh] flex items-center justify-center px-4 py-12" dir="rtl">
	<div class="w-full max-w-md rounded-3xl border border-white/10 bg-[#0f172a] p-8 shadow-2xl">
		...
		<!-- הגיעו מאתר-אח דרך גשר ה-SSO: מסבירים שההתחברות/ההצטרפות כאן מחזירה
		     אותם לשם אוטומטית, ושחברות בווצאפ אינה חשבון -->
		{#if data.via}
			<div role="status" class="mb-6 rounded-xl border border-purple-400/30 bg-purple-500/10 px-4 py-3 text-center">
				<p class="mb-1 text-sm font-bold text-purple-100">🕊️ כניסה לאתר {data.via}</p>
				<p class="text-xs leading-relaxed text-gray-300">
					אחרי ההתחברות תוחזרו לשם אוטומטית. אין לכם עדיין חשבון? חברות בקבוצות הווצאפ אינה חשבון באתר, אבל כניסה עם Google או Facebook יוצרת אחד בלחיצה.
				</p>
			</div>
		{/if}
		...
		{#if data.ssoName}
			...
			<button
				...
				class="login-grad mb-2 flex w-full items-center justify-center gap-2 rounded-2xl px-4 py-3.5 font-bold text-white transition hover:brightness-110 disabled:opacity-60"
			>
				{#if loading === 'sso'}
