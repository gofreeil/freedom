...
<div class="flex items-center gap-2">
                ...
                {#if user}
                    ...
                    >
                        <span class="flex h-6 w-6 items-center justify-center rounded-full bg-linear-to-br/oklch from-amber-500 to-pink-600 text-xs">{user.isSuperAdmin ? '🛡️' : '👤'}</span>
                        <span class="hidden sm:inline max-w-[120px] truncate">{user.isSuperAdmin ? 'ניהול הרשת' : (user.name || user.email)}</span>
                    </a>
                {:else}
                    <a
                        href="/login"
                        class="flex items-center gap-1.5 rounded-lg bg-linear-to-r/oklch from-amber-500 to-pink-600 hover:from-amber-400 hover:to-pink-500 px-3 py-2 text-sm font-bold text-white transition-all"
                    >
                        <span>🕊️</span>
